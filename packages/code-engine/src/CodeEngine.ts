...
export default class CodeCompiler {
  ...
  public getCompilerCode() {
    return  this.es5 ? this.getEs5FunctionStr() : this.getFunctionStr()
  }
  /**
   * 获取转换后的es5 代码字符串
   */
  public getEs5FunctionStr(): string {
    return transform(this.getFunctionStr(), {
      presets: [
  ...
  public codeIsExpression(code: string): boolean {
    ...
    return expression && expression.body.body.length === 1 && expression.body.body[0].type === "ExpressionStatement"
  }
  /**
   * 变量AST 并对节点进行对应转化
   */
